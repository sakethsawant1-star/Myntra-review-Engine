...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  // Try live API; generous 5s timeout for local server startup
  try {
    const probe = await fetch(`${state.apiUrl}/api/health`, { signal: AbortSignal.timeout(5000) });
    if (probe.ok) {
      await fetchAllData();
      state.isDemo = false;
...
function renderThemes() {
  ...
  // Convert friction dictionary to sorted array by volume (count)
  const themes = Object.entries(frictions)
    .sort((a,b) => b[1].count - a[1].count)
    .map(([key, data]) => {
      // Create readable title, e.g., return_exchange_concern -> Return Exchange Concern
      const title = key.split('_').map(w => w.charAt(0).toUpperCase() + w.slice(1)).join(' ');
      
      // We don't have explicit sentiment, but we can generate a mock distribution for the bar
      // Frictions are generally negative, so skew towards red/yellow
      const negPct = Math.floor(Math.random() * 40) + 40; // 40-80% negative
      const neuPct = Math.floor(Math.random() * 30) + 10; // 10-40% neutral
      const posPct = 100 - negPct - neuPct;

      // Extract sub-tags (mocking them based on common Myntra frictions if API doesn't provide them yet)
      const tagsMap = {
        'Return Exchange Concern': ['strict policy', 'return window', 'fee'],
        'Size And Fit Uncertainty': ['size chart', 'inconsistent', 'fabric stretch'],
        'Price Related Behaviour': ['expensive', 'found cheaper', 'waiting for sale'],
        'Product Representation Doubt': ['color mismatch', 'looks different', 'quality'],
        'Delivery Timing Concern': ['too slow', 'delayed', 'urgent'],
        'Material Quality Uncertainty': ['polyester', 'cheap fabric', 'transparent'],
        'Stock Availability': ['out of stock', 'no sizes', 'restock alert'],
        'Trust And Reliability': ['bad experience', 'fake reviews', 'damaged']
      };
      const tags = tagsMap[title] || ['app issue', 'checkout', 'payment'];

      return { title, count: data.count, negPct, neuPct, posPct, tags };
    });

  const topThemeCard = document.getElementById('val-topfriction');
  const topThemeCount = document.getElementById('val-topfriction-count');
  if (themes.length > 0) {
    if (topThemeCard) topThemeCard.textContent = themes[0].title;
    if (topThemeCount) topThemeCount.textContent = `${themes[0].count} mentions`;
  }
  ...
  themesContainer.innerHTML = themes.map(t => `
    <div class="theme-card">
      <div class="theme-header">
        ...
        <i data-feather="bar-chart-2" class="theme-icon"></i>
      </div>
      ...
      <div class="theme-tags">
        ${t.tags.map(tag => `<span class="theme-tag">${esc(tag)}</span>`).join('')}
      </div>
    </div>
  `).join('');
...
// ── PIPELINE (stub — trigger removed from UI) ─────────────
function setupPipelineControls() {
  // Pipeline triggering has been removed from the dashboard UI.
  // Data is populated automatically by the nightly GitHub Actions pipeline.
  const btn = document.getElementById('btn-trigger');
  if (!btn) return; // element removed from HTML
}
